...
def forward_layer(layer, position_ids, attention_mask, bs, device, in_q, out_q):
    ...
    rotary_emb = Phi3RotaryEmbedding(layer.config).to(device)

    # Register hooks
    done_qkv = register_H_hook(layer.self_attn.qkv_proj, device)
    done_o = register_H_hook(layer.self_attn.o_proj, device)
    done_up = register_H_hook(layer.mlp.gate_up_proj, device)
    ...
    while True:
        ...
        for i in range(len(dev_emb) // bs):
            ...
            with torch.cuda.stream(torch.cuda.Stream()):
                # Generate position embeddings for this batch
                batch_position_ids = position_ids[:batch.size(0)]
                position_embeddings = rotary_emb(batch, batch_position_ids)
                ...
                output = layer(
                    ...
                    position_embeddings=position_embeddings  # Pass the position embeddings
                )[0]
...
def main(args):
    ...
    for transformer_layer_index in range(len(model.model.layers)):
        ...
        assert (len([m for m in transformer_layer.modules() if isinstance(m, torch.nn.Linear)]) == 4)
